...
download it as a high-resolution PNG. Everything runs client-side: no backend, no account, no
upload.
...
- **Artwork** fills the card, with a horizontal scrim protecting the text column (adjustable, plus
  zoom and pan).
...
`maxDelta` is the largest per-channel difference across every pixel. It returns 0 both with and
without artwork. Downloaded files were also opened from disk and inspected at 1×, 2× and 3×.

## Images and privacy

Uploads never leave the device. A selected file is decoded, re-encoded through a canvas (capping the
long edge and **stripping EXIF, including GPS**) and stored as a blob in this browser's IndexedDB.
There is no server and no network request involved, so images saved in one person's browser are not
reachable from anyone else's — the browser's origin/profile boundary is what enforces it, rather
than an access-control check that could be got wrong.

The trade-off worth stating plainly: storage is per-browser, so images do not follow you to another
...
src/
  ...
  lib/
    ...
    images.ts            IndexedDB library for artwork/avatar/logo
    render.ts            the single paint entry point, preview + export
    share.ts             download / clipboard / Web Share
